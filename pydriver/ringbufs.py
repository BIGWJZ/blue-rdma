...
class Ringbuf:
    def __init__(self, backend_mem, bar_host, is_h2c, head_csr_addr, tail_csr_addr, desc_size=32, ringbuf_len=128) -> None:
    ...
    def sync_pointers(self):
        if (self.is_h2c):
            self.bar_host.write_csr_blocking(self.head_csr_addr, self.head)
            new_tail = self.bar_host.read_csr_blocking(self.tail_csr_addr)
            self.set_tail_pointer_with_guard_bit(new_tail)
        else:
            self.bar_host.write_csr_blocking(self.tail_csr_addr, self.tail)
            new_head = self.bar_host.read_csr_blocking(self.head_csr_addr)
    ...
    def write_backend_mem(self, addr, len, data):
        self.backend_mem[addr : addr + len] = data
        
    def read_backend_mem(self, addr, len) -> bytes:
        return self.backend_mem[addr : addr + len]
...
class RingbufCommandReqQueue:
    def __init__(self, backend_mem, start_pa, bar_host) -> None:
        self.rb = Ringbuf(backend_mem=backend_mem, bar_host=bar_host, is_h2c=True,
        ...
        bar_host.write_csr_blocking(
            CSR_ADDR_CMD_REQ_QUEUE_ADDR_HIGH, start_pa >> 32)
...
class RingbufCommandRespQueue:
    def __init__(self, backend_mem, start_pa, bar_host) -> None:
        self.rb = Ringbuf(backend_mem=backend_mem, bar_host=bar_host, is_h2c=False,
                          head_csr_addr=CSR_ADDR_CMD_RESP_QUEUE_HEAD, tail_csr_addr=CSR_ADDR_CMD_RESP_QUEUE_TAIL)
        print("Blue-RDMA INFO: cmdRespQ init, buffer start pa:0x%x head_csr_addr:%d, tail_csr_addr:%d"%
              (start_pa, CSR_ADDR_CMD_RESP_QUEUE_HEAD, CSR_ADDR_CMD_RESP_QUEUE_TAIL))
        bar_host.write_csr_blocking(
            CSR_ADDR_CMD_RESP_QUEUE_ADDR_LOW, start_pa & 0xFFFFFFFF)
        bar_host.write_csr_blocking(
            CSR_ADDR_CMD_RESP_QUEUE_ADDR_HIGH, start_pa >> 32)
...
class RingbufSendQueue:
    def __init__(self, backend_mem, start_pa, bar_host) -> None:
        ...
        print("Blue-RDMA INFO: SQ init, buffer start pa:0x%x head_csr_addr:%d, tail_csr_addr:%d"%
              (start_pa, CSR_ADDR_SEND_QUEUE_HEAD, CSR_ADDR_SEND_QUEUE_TAIL))
        bar_host.write_csr_blocking(
            CSR_ADDR_SEND_QUEUE_ADDR_LOW, start_pa & 0xFFFFFFFF)
        bar_host.write_csr_blocking(CSR_ADDR_SEND_QUEUE_ADDR_HIGH, start_pa >> 32)
...
class RingbufMetaReportQueue:
    def __init__(self, backend_mem, start_pa, bar_host) -> None:
        ...
        print("Blue-RDMA INFO: MetaRptQ init, buffer start pa:0x%x head_csr_addr:%d, tail_csr_addr:%d"%
              (start_pa, CSR_ADDR_META_REPORT_QUEUE_HEAD, CSR_ADDR_META_REPORT_QUEUE_TAIL))
        bar_host.write_csr_blocking(
            CSR_ADDR_META_REPORT_QUEUE_ADDR_LOW, start_pa & 0xFFFFFFFF)
        bar_host.write_csr_blocking(
            CSR_ADDR_META_REPORT_QUEUE_ADDR_HIGH, start_pa >> 32)
